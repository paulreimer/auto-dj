...
def crossfade(audio1, audio2, start1, start2, length, method=linear):
	...
	for i, sample1, sample2 in zip(list(range(start1, start1+length)), audio1[start1:start1+length], audio2[start2:start2+length]):
...
if __name__ == '__main__':
	...
	if(bpm2 != bpm1):
		print('Time stretching with rate ', old_div(bpm1,bpm2))
		audio2_stretched = prb.time_stretch(audio2, 44100, old_div(bpm1,bpm2))
	
	# Cross-fading
	start1 = int(44100.0 * (phase1 + 537*(60./bpm1)))
	start2 = int(44100.0 * (old_div(phase2 * bpm2,bpm1) + 1*(60./bpm1))) #bpm1 because audio2 has been stretched!
